...
export default function Filters({values, setPage, setCategory}) {
    ...
    useEffect(() => {
        const buttons = document.querySelectorAll('.category-container-desktop__button')
    ...
    <>
      ...
      <div className='category-container-mobile'>
        ...
        <div className="category-container-mobile__buttons openbox">
          <ul>
              {values.map((element) => {
              return <li key={element.id}><button className='category-container-mobile__button' value={element.valor} key={element.id} onClick={(e) => {changeCategory(e); handleClick()}}>{element.nombre}</button></li>
              })}
          </ul>
      </div>
    </div>
